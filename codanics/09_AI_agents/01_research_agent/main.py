...
from langchain_anthropic import ChatAnthropic # for using Anthropic's chat models

from pydantic import BaseModel # for data validation and parsing
from langchain_core.output_parsers import PydanticOutputParser # to parse LLM outputs into Pydantic models
...
# to create structured prompts for the LLM
from langchain_core.prompts import ChatPromptTemplate # when user write its prompt, it will refine user prompt to make it better
from langchain.agents import create_tool_calling_agent, AgentExecutor # functions to create an agent and execute it

from tools import search_tool, wiki_tool, save_tool # tools for the agent to use
from datetime import datetime # for generating timestamps

load_dotenv() # Load environment variables

# Fix SSL certificate issue by removing problematic SSL_CERT_FILE if it exists - bypass SSL verification
...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            Always save answer in text file which will contain 
            Research Query: "query here"
            Generated on: "timestanp here"
            ==================================================
            ...
            Tools Used: "tool name 1", "tool name 2"

            Above is an example format. use this format to save your research.

            Wrap the final output in this format and provide no other text:
            {format_instructions}
            """,
        ),
        ("placeholder", "{chat_history}"),      # Placeholder for chat history - to maintain context
...
agent_executor = AgentExecutor(
    ...
    tools=tools, 
    verbose=True,
    max_iterations=10,  # Increase max iterations to allow more research steps
