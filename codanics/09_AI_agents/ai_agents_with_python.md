...
1. **Conceptual** – Define _intelligent / autonomous agents_ and recognise the major agent architectures.
...
> _An intelligent agent is an entity that perceives its environment, takes actions autonomously to achieve goals, and can improve its performance over time._
...
| ----------------------------- | ---------------------------------------------------------- | -------------------------------- | --------------------------------------------- |
| **Reflex / rule-based**       | Hard-coded _condition → action_ rules                      | small, fully observable tasks    | Thermostat (if temp > 36°C → turn on AC)      |
...
_Try modifying the rules to see how behaviour changes._
...
- a chat-model,
- a list of _tools_ (Python callables),
- short-term memory.
...
The language model decides _when_ to call the search-tool and integrates results before answering.
...
- **Evaluation** – Record traces and define _task-level_ success metrics, not just token accuracy.
...
2. [LangChain _Build an Agent_ tutorial (2025-03)](https://python.langchain.com/docs/tutorials/agents/)
